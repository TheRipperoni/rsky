...
use rsky_lexicon::com::atproto::server::{
    CreateAccountOutput, CreateInviteCodeOutput, CreateSessionOutput, DescribeServerOutput,
    DescribeServerRefContact, DescribeServerRefLinks,
};
...
// #[tokio::test]
// async fn test_static_resources() {
...
//     assert_eq!(response_status, Status::Ok);
//
//     let output = response.into_json::<DescribeServerOutput>().await.unwrap();
//     let expected = DescribeServerOutput {
//         invite_code_required: None,
//         phone_verification_required: None,
//         available_user_domains: vec![".rsky.com".to_string()],
//         links: DescribeServerRefLinks {
//             privacy_policy: None,
//             terms_of_service: None,
//         },
//         contact: DescribeServerRefContact {
//             email: Some("contact@ripperoni.com".to_string()),
//         },
//         did: "did:web:rsky.com".to_string(),
//     };
//     assert_eq!(output, expected);
// }

#[tokio::test]
async fn test_create_bsky_profile() {
    let postgres = common::get_postgres().await;
    let client = common::get_client(&postgres).await;
    let input = json!({
        "useCount": 1
...
async fn test_create_invite_code_and_account() {
    ...
    let create_account_input = json!({
        ...
        "handle": "dummyabcdefghi1.rsky.com",
        "password": "password",
    ...
    let response = client
        ...
        .header(ContentType::JSON)
        .body(create_account_input.to_string())
        .dispatch()
        .await;
    let response_status = response.status();
    assert_eq!(response_status, Status::Ok);
    let output = response.into_json::<CreateAccountOutput>().await.unwrap();
    let access_jwt = "Bearer ".to_string() + output.access_jwt.as_str();
    let actor = output.did;

    // // Get Preferences
    let response = client
        .get("/xrpc/app.bsky.actor.getPreferences")
        .header(Header::new("Authorization", access_jwt.clone()))
        .dispatch()
        .await;
    let response_status = response.status();
    assert_eq!(response_status, Status::Ok);
    let output = response.into_json::<GetPreferencesOutput>().await.unwrap();
    assert_eq!(output.preferences.is_empty(), true);

    // Put Preferences
    let put_preferences_input = json!(
        {
            "preferences":[{"$type":"app.bsky.actor.defs#personalDetailsPref","birthDate":"1995-05-06T00:00:00.000Z"}]
        }
    );
    let response = client
        ...
        .header(ContentType::JSON)
        .header(Header::new("Authorization", access_jwt.clone()))
        .body(put_preferences_input.to_string())
        .dispatch()
        .await;
    let response_status = response.status();
    assert_eq!(response_status, Status::Ok);
    ...
    // Get Profile
    let profile_uri = format!("/xrpc/app.bsky.actor.getProfile?actor={actor}");
    let response = client
        .get(profile_uri)
        .header(Header::new("authorization", access_jwt.clone()))
        .dispatch()
    ...
    let response_status = response.status();
    assert_eq!(response_status, Status::Ok);
    let output = response.into_json::<ProfileViewDetailed>().await.unwrap();
    assert_eq!(output.handle, "dummaccount.rsky.com".to_string());

    // Create Post
    let create_post_input = json!(
        {
            "repo":actor,
            "writes":[
                {
                    ...
                    "value":{
                        ...
                        "createdAt":"2025-05-02T16:26:05.792Z",
                        "text":"test",
                        "langs":["en"]
                    }
                }
    ...
    let response = client
        ...
        .header(ContentType::JSON)
        .header(Header::new("authorization", access_jwt.clone()))
        .body(create_post_input.to_string())
        .dispatch()
        .await;
    assert_eq!(response_status, Status::Ok);
    let output = response.into_json::<ApplyWritesOutput>().await.unwrap();
    let uri = output.results.get(0).unwrap().uri.clone();

    // Get Post Thread
    let post_thread_endpoint = format!("xrpc/app.bsky.feed.getPostThread?uri={uri}&depth=0");
    let response = client
        .get(post_thread_endpoint)
        .header(Header::new("authorization", access_jwt.clone()))
        .body(create_account_input.to_string())
        .dispatch()
        .await;
